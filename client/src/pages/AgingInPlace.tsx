...
function Header() {
  ...
  return <header className="ds-header aip-header">
    <div className="ds-header__top"><a href="/" className="ds-brand"><img src={wideLogo} alt="Downsize Baltimore" /></a><ButtonLink>Talk to Mary</ButtonLink><button ref={menuButtonRef} type="button" className="ds-menu" aria-label={open ? "Close navigation" : "Open navigation"} aria-expanded={open} aria-controls="aging-primary-navigation" onClick={() => setOpen(!open)}>{open ? <X size={22} /> : <Menu size={22} />}</button></div>
    <nav id="aging-primary-navigation" className={`ds-nav ${open ? "is-open" : ""}`} aria-label="Primary navigation" onKeyDown={(event) => { if (event.key === "Escape") { setOpen(false); menuButtonRef.current?.focus(); } }}>{navItems.map(([label, href]) => <a key={label} href={href} aria-current={href === "/aging-in-place" ? "page" : undefined} onClick={() => setOpen(false)}>{label}</a>)}<span className="ds-nav-cta-mobile"><ButtonLink>Talk to Mary</ButtonLink></span></nav>
  </header>;
}
...
function Footer() {
  return <footer className="ds-footer" id="contact" style={{ backgroundImage: `linear-gradient(rgba(16,42,67,.98),rgba(16,42,67,.98)),url(${paperTexture})` }}>
    ...
    <div className="ds-footer__bottom"><div className="ds-footer__links">{footerNavItems.map(([label, href]) => <a key={label} href={href} aria-current={href === "/aging-in-place" ? "page" : undefined}>{label}</a>)}</div><span>© 2026 Downsize Baltimore. All rights reserved.</span><span>Real estate services provided in affiliation with a licensed brokerage.</span></div>
  </footer>;
}
...
export default function AgingInPlace() {
  return <div className="aip-page"><Header /><main id="main-content" tabIndex={-1}>
    <section className="aip-hero aip-hero--split">
      ...
      <div className="aip-hero__photo" role="img" style={{ backgroundImage: `url(${heroPhoto})` }} aria-label="Mary Lynch seated at a table in a bright home" />
    </section>

    <section className="aip-stats" style={{ backgroundImage: `url(${paperTexture})` }}><h2 className="sr-only">The gap between wanting to stay and being ready to stay</h2><div className="aip-section-label" aria-hidden="true"><Anchor size={15} /><span /> THE GAP BETWEEN WANTING TO STAY AND BEING READY TO STAY</div><div className="aip-stats__grid"><article><strong>75%</strong><p>of adults age 50+ say they want to remain in their current homes as they age.</p></article><article><strong>Less than 4%</strong><p>of U.S. homes have a no-step entrance, single-floor living, and doors and hallways wide enough for a wheelchair.</p></article></div><p className="aip-source">National sources: <a href="https://www.aarp.org/pri/topics/livable-communities/housing/2024-home-community-preferences/">AARP’s 2024 Home and Community Preferences Survey</a>; <a href="https://www.jchs.harvard.edu/blog/housing-americas-older-adults-four-problems-we-must-address">Harvard Joint Center for Housing Studies analysis of national housing data</a>. Baltimore’s older, multi-level homes and rowhomes may present additional challenges for aging in place.</p></section>

    <section className="aip-personal" id="meet-mary"><div className="aip-personal__copy"><p className="aip-eyebrow">Why this work matters to Mary</p><h2>For Me,<br /><i>This Is Personal.</i></h2><div className="aip-gold-rule" /><p>Mary’s interest in aging in place began while helping care for her grandfather. He had lived in his home for decades, but the house itself eventually became part of the challenge: laundry was in the basement, bedrooms and bathrooms were upstairs, and steps were required at the entrances.</p><p>That experience made Mary realize how often families begin thinking about housing only after circumstances have changed.</p><p>Today Mary is the <strong>Founder and Chair of the National Aging in Place Council, Greater Baltimore Chapter</strong>, bringing together professionals across housing, healthcare, finance, home care, wellness, and other aging-related services.</p><p>She also leads educational programs throughout Greater Baltimore, including <strong>Should I Stay or Should I Go?</strong>, designed to help older adults and families understand their options before a crisis makes the decision for them.</p><a className="aip-text-link" href="/meet-mary">Read Mary’s Story <ArrowUpRight size={16} /></a></div><div className="aip-personal__visual"><div className="aip-personal__photo-frame"><div className="aip-personal__photo-crop"><img className="aip-personal__photo-source" src={personalPhoto} alt="Mary Lynch and Pop together on a boat" /></div></div><div className="aip-personal__seal"><Anchor size={28} /><span>Planning before<br />the pressure.</span></div></div></section>

    <section className="aip-assess" style={{ backgroundImage: `linear-gradient(rgba(242,237,226,.96),rgba(242,237,226,.96)),url(${paperTexture})` }}><div className="aip-section-head"><div><p className="aip-eyebrow">Start with the home</p><h2>Can Your Home Continue<br /><i>to Work for You?</i></h2></div><p>A thoughtful review looks at the daily details that make a home safer, easier, and more sustainable.</p></div><div className="aip-assess-grid">{assessmentTopics.map(([title, copy, Icon]) => <article key={title as string}><Icon size={23} strokeWidth={1.25} /><h3>{title as string}</h3><p>{copy as string}</p></article>)}</div><a className="aip-text-link" href="#self-evaluation">Take the home and lifestyle self-evaluation <ArrowUpRight size={16} /></a></section>

    <section className="aip-story"><div className="aip-story__image" role="img" aria-label="Mary Lynch talking with an older adult in a bright office" style={{ backgroundImage: `url(${consultationPhoto})` }}><span>02 / A CONVERSATION IN CONTEXT</span></div><div className="aip-story__copy"><p className="aip-eyebrow">The whole picture matters</p><h2>Your Home Is Only One Part <i>of the Plan.</i></h2><p>Successful aging in place also means thinking about the life around the house.</p><div className="aip-life-list">{["Healthcare and future care needs", "Transportation", "Friends, family, and social connection", "Financial sustainability", "Access to groceries, services, and activities", "Technology and support", "What happens if driving or mobility changes"].map(item => <span key={item}><Check size={14} />{item}</span>)}</div><blockquote>Aging in place is not simply a renovation project. It is a plan for the life around the house.</blockquote></div></section>
