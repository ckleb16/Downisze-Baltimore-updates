...
const resources = [["Am I Ready to Downsize?", "A short self-assessment for lifestyle, maintenance, space, finances, and future needs."], ["Your Downsizing Plan", "A printable worksheet for destination, budget, timeframe, needs, and concerns."], ["10 Ways to Start Tackling the Stuff", "A practical guide for reducing clutter without doing everything at once."], ["Comparing Your Next-Home Options", "A guide to smaller homes, condos, villas, 55+ communities, and aging in place."], ["Aging in Place: Remodel or Move?", "Questions to help determine whether your current home can support future needs."], ["Downsizing Checklist", "A printable step-by-step checklist for the pieces of the transition."]];
...
export default function Downsizing() {
  ...
  return <div className="ds-page"><Header /><main>
    ...
    <section className="ds-destination"><div className="ds-destination__image" style={{ backgroundImage: `url(${roomPhoto})` }}><span className="ds-image-label">02 / A POSSIBLE NEXT HOME</span></div><div className="ds-destination__copy"><p className="ds-eyebrow">What are you moving toward?</p><h2>Downsizing Is About More Than a <i>Smaller House.</i></h2><p>For many people, the goal isn’t simply less square footage.</p><p>It may be making room for:</p><div className="ds-possibilities">{possibilities.map((item, i) => <span key={item}><b>0{i + 1}</b>{item}</span>)}</div><blockquote>The goal is not simply to give something up. It is to decide what you want to make room for.</blockquote></div></section>

    <section className="ds-steps"><div className="ds-section-head"><div><p className="ds-eyebrow ds-eyebrow--gold">A clear path forward</p><h2>One Step<br /><i>at a Time.</i></h2></div><p>You don’t have to solve everything today.<br />We put the pieces in the right order.</p></div><div className="ds-step-grid">{steps.map(([num, title, copy], index) => <article className={`ds-step-card ${openStep === index ? "is-open" : ""}`} key={num} role="button" tabIndex={0} onClick={() => setOpenStep(openStep === index ? null : index)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); setOpenStep(openStep === index ? null : index); } }}><div className="ds-step-card__top"><span>{num}</span><Anchor size={17} /></div><h3>{title}</h3><p>{copy}</p></article>)}</div></section>
    ...
    <section id="resources" className="ds-resources" style={{ backgroundImage: `linear-gradient(rgba(16,42,67,.97),rgba(16,42,67,.97)),url(${contourTexture})` }}><div className="ds-resources__head"><div><p className="ds-eyebrow ds-eyebrow--gold">A little more clarity</p><h2>Want to Dig<br /><i>a Little Deeper?</i></h2></div><p>These resources are designed to help you take the next step when you’re ready — without trying to do everything at once.</p></div><div className="ds-resource-grid">{resources.map(([title, copy], i) => <article key={title}><span>0{i + 1}</span><h3>{title}</h3><p>{copy}</p></article>)}</div><div className="ds-button-row"><ButtonLink variant="light" href="/resource-center">Visit the Resource Center</ButtonLink><ButtonLink variant="outline" href="/resource-center#club">Learn About the Downsizers Club</ButtonLink></div></section>
